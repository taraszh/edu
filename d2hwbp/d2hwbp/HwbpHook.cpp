...
void HwbpHook::SetExceptionHandler(PVECTORED_EXCEPTION_HANDLER pVecExcHandler) {
	pException = AddVectoredExceptionHandler(1, pVecExcHandler);
}

int HwbpHook::GetFreeReg(unsigned long long* mask) {
	int freeReg = -1;
	for (int i = 0; i < 4; i++) {
		if ((*mask & (1ULL << (i * 2))) == 0) {
			freeReg = i;
...
int HwbpHook::SetHook(void* source, void* destination) {
	...
	Freeze();
	do {
		if (entry.th32OwnerProcessID == dwOwnerPID) {
...
int HwbpHook::SetHook(void* source, void* destination, HANDLE* hThread, int* reg) {
	CONTEXT context;
	ZeroMemory(&context, sizeof(context));
	context.ContextFlags = CONTEXT_DEBUG_REGISTERS;
...
int HwbpHook::UnsetHook(void* source) {
	THREADENTRY32 te32;
	HANDLE hThread = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0);
	if (hThread == INVALID_HANDLE_VALUE)
		return ERROR_ENUM_THREAD_START;
...
int HwbpHook::UnsetHook(void* source, HANDLE* hThread) {
	CONTEXT context;
	ZeroMemory(&context, sizeof(context));
	context.ContextFlags = CONTEXT_DEBUG_REGISTERS;
	if (!GetThreadContext(*hThread, &context))
...
Vector<HookRecord*>* HwbpHook::GetInfo() {
	return info;
}

HookRecord* HwbpHook::GetRecordBySource(void* source) {
